        ...
        for key, value in result_output_dict.items():
            if key == "alias" and isinstance(value, dict):
                if "name" in value and isinstance(value["name"], list):
                    ...
                    value["name"] = ", ".join(value["name"])
                formatted_items.append(
                    f"{indent}{key} = {format_dict(value, indent_level + 1)}"
                )
            else:
                if key in ["zone_id", "evaluate_target_health"]:
...
def process_single_input_data(user_input_str):
    ...
    with open("input_data_parsed.json", "w") as file:
    ...
    with open("Final Formatted Output Data.txt", "w") as file:
    ...
    final_output.to_csv(r"Final Formatted Output Data.csv", index=False)
...
def process_multiple_input_data(user_input_str):
    ...
    if user_input_dict:
        ...
        final_output_df.to_csv(r"Final Formatted Output Data.csv", index=False)
